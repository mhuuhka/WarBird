...
def check_wanted_angle():
	global set_y_angle
	global set_x_angle

	if forward != '1' and backward != '1':		# No Forward/Backward
		set_y_angle = 0
	elif forward == '1' and backward != '1':	# Forward
		set_y_angle = 10
	elif forward != '1' and backward == '1':	# Backward
		set_y_angle = -10
	else:
		set_y_angle = 0

	if left != '1' and right != '1':		# No Left/Right
		set_x_angle = 0
	elif left == '1' and right != '1':		# Left
		set_x_angle = -10
	elif left != '1' and right == '1':		# Right
		set_x_angle = 10
	else:
		set_x_angle = 0
...
def motor_control():
	...
	while True:
		if forward == '1':
			set_y_angle = -10
			while yRot != set_y_angle:
				if yRot > set_y_angle:
					motorcontrol(5,5,-5,-5)
				elif yRot < set_y_angle:
					motorcontrol(-5,-5,5,5)
			if yRot == set_y_angle:
...
socketIO.emit('DRONEboot', drone_ip)
socketIO.on('IOanswer', on_drone_server_response)
